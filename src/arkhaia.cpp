...
arma::mat LSSA_LFI_arma(const arma::mat & x, int n_iter, int intercept) {
    ...
    if (intercept == 1) {
        ...
        aic[0] = 2 * (3 + 1) + log(epsilon2 / nr) * nr + nr + nr * log(2 * M_PI);
    }
    ...
    if (n_iter > 1){
        for (int j = 1; j < n_iter; j++) {
            ...
            resid = LSSA_resid_arma(x0, freq1, 0);
...
// //' @importFrom Rcpp sourceCpp
// // [[Rcpp::export]]
// arma::mat LSSA_LFI_multi_arma2(arma::mat & x, arma::vec & start_idx, arma::vec & x_rows, int n_iter, int intercept) {
